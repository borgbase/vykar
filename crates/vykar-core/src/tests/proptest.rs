...
mod chunker {
    ...
    ///
    /// Exercise even and legacy odd average/maximum sizes. Keep the minimum
    /// even because FastCDC 4's paired scan can cut one byte below an odd
    /// minimum; exact compatibility for odd minima is tested separately.
