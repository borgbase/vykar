...
/// A repository created with an odd maximum must keep its original forced
/// cuts. In particular, bounding the reader must not round above that cap.
#[test]
fn legacy_odd_maximum_preserves_pinned_chunks() {
    ...
    assert_eq!(chunk_data(&data, &config), expected);
    let chunks: Vec<_> = chunk_stream_bounded(Cursor::new(&data).take(16384), &config)
        .map(|result| {
...
/// Cover each odd parameter, including configs decoded from persisted msgpack
/// without YAML validation. Both file and command-dump streams must match the
/// pinned legacy dependency on random data and forced-cut data.
#[test]
fn legacy_parameters_match_fastcdc4_for_bounded_and_unbounded_streams() {
    for (min_size, avg_size, max_size) in [
        ...
        let encoded = rmp_serde::to_vec(&ChunkerConfig {
            min_size,
            avg_size,
            max_size,
        })
        .unwrap();
        let config: ChunkerConfig = rmp_serde::from_slice(&encoded).unwrap();
        for data in [seeded_data(0x0123_4567_89ab_cdef, 20000), vec![0; 20000]] {
            for limit in [
                0, 1, 255, 257, 1023, 1024, 1025, 3001, 4094, 4095, 4096, 4097, 19999, 20001,
            ] {
                let expected: Vec<_> = fastcdc_legacy::v2020::StreamCDC::new(
                    Cursor::new(&data).take(limit),
                    min_size as usize,
                    avg_size as usize,
                    max_size as usize,
                )
                .map(|result| {
                    let chunk = result.unwrap();
                    (chunk.offset, chunk.length, chunk.data)
                })
                .collect();
                let collect = |result: Result<fastcdc::v2020::ChunkData, fastcdc::v2020::Error>| {
                    let chunk = result.unwrap();
                    assert!(chunk.length <= max_size as usize);
                    (chunk.offset, chunk.length, chunk.data)
                };
                let unbounded: Vec<_> = chunk_stream(Cursor::new(&data).take(limit), &config)
                    .map(collect)
                    .collect();
                assert_eq!(unbounded, expected);
                let mut reader = Cursor::new(&data);
                let bounded: Vec<_> = chunk_stream_bounded((&mut reader).take(limit), &config)
                    .map(collect)
                    .collect();
                assert_eq!(bounded, expected);
                assert_eq!(reader.position(), limit.min(data.len() as u64));
            }
        }
    }
}
...
#[test]
fn legacy_stream_preserves_io_error_kind() {
    ...
    let error = chunk_stream(FailingReader, &config)
        .next()
        .unwrap()
        .unwrap_err();
    assert!(matches!(error, fastcdc::v2020::Error::IoError(error)
