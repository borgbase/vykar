...
/// Wire every MainWindow callback. Thin dispatcher over the per-domain
/// `wire_*` functions below; add new callbacks to the domain they belong to
...
pub(crate) fn wire_callbacks(
    ...
    wire_config_editor(ui, &app_tx);
    wire_backup_actions(ui, &app_tx);
    wire_navigation(ui, &app_tx);
    wire_snapshots(ui, &app_tx, &ui_tx);
    wire_find(ui, &app_tx);
    wire_footer_links(ui, &app_tx);
    wire_cancel(ui, &ui_tx, cancel_requested);
}

/// Advanced (config editor) page: open/switch/save/discard/reload plus the
/// repository recovery buttons (clear locks / clear sessions).
fn wire_config_editor(ui: &MainWindow, app_tx: &Sender<AppCommand>) {
    let tx = app_tx.clone();
    ...
    ui.on_save_and_apply_clicked(move || {
        if let Some(u) = ui_weak.upgrade() {
            ...
            // Immediate feedback: the worker's result event overwrites this with
            // success/error, but if the command queues behind a long-running
            // operation the user still sees that the save was accepted.
            u.set_editor_status("Applying configuration…".into());
...
/// Backup triggers and source-row expansion (Overview + Sources pages).
fn wire_backup_actions(ui: &MainWindow, app_tx: &Sender<AppCommand>) {
    let tx = app_tx.clone();
    ...
    // Retry a single repo after a load failure (Overview error banner).
    let tx = app_tx.clone();
    ui.on_retry_repo(move |name| {
        let name = name.to_string();
        if !name.is_empty() {
            let _ = tx.send(AppCommand::FetchRepoInfo { repo_name: name });
        }
    });
...
/// Sidebar navigation: page selection and repo+page selection.
fn wire_navigation(ui: &MainWindow, app_tx: &Sender<AppCommand>) {
    let ui_weak = ui.as_weak();
...
/// Snapshots page: refresh/prune, sorting, row selection, and the
/// restore/mount/diff actions driven off the current selection.
fn wire_snapshots(ui: &MainWindow, app_tx: &Sender<AppCommand>, ui_tx: &Sender<UiEvent>) {
    let tx = app_tx.clone();
    let ui_weak = ui.as_weak();
...
/// Find page search callback.
fn wire_find(ui: &MainWindow, app_tx: &Sender<AppCommand>) {
    let tx = app_tx.clone();
    let ui_weak = ui.as_weak();
...
/// Footer controls: the mount "Stop" button. The mount-URL and update links
/// open via Slint's built-in `Platform.open-url`, so they need no Rust wiring.
fn wire_footer_links(ui: &MainWindow, app_tx: &Sender<AppCommand>) {
    let tx = app_tx.clone();
...
/// Cancel button — mirrors the tray "Cancel" menu item.
fn wire_cancel(ui: &MainWindow, ui_tx: &Sender<UiEvent>, cancel_requested: Arc<AtomicBool>) {
